...
# Experience distribution data - DETROIT
experience_distribution = Dict(
    "<1 Year" => 12.24, "1 Year" => 9.79, "2 Years" => 5.34,
    "3 Years" => 4.13, "4 Years" => 3.83, "5 Years" => 3.74,
    "6-10 Years" => 12.80, "11-15 Years" => 13.59, "16-20 Years" => 16.07,
    "21-25 Years" => 9.30, "26-30 Years" => 4.83, "31-35 Years" => 3.40,
    "36-40 Years" => 0.87, ">40 Years" => 0.14
)
...
degree_distribution = [0.0363, 0.2196, 0.7107, 0.0221, 0.0113]
...
@NLobjective(model, Max, sum((delta * (b + c * (alpha * teacher_data.ExperienceYears[i] + beta * teacher_data.DegreeLevelNumeric[i]))^rho +
                               (1 - delta) * teacher_data.ExperienceYears[i]^rho)^(1/rho) for i in 1:nrow(teacher_data)))

# Use the full budget exactly
@NLconstraint(model, sum(b + c * (alpha * teacher_data.ExperienceYears[i] + beta * teacher_data.DegreeLevelNumeric[i]) for i in 1:nrow(teacher_data)) == value(total_budget))

# Solve the optimization problem
optimize!(model)

optimized_total_salary = sum(value(b) + value(c) * (value(alpha) * teacher_data.ExperienceYears[i] + value(beta) * teacher_data.DegreeLevelNumeric[i]) for i in 1:nrow(teacher_data))
optimized_total_effectiveness = sum(
    (value(delta) * (value(b) + value(c) * (value(alpha) * teacher_data.ExperienceYears[i] + value(beta) * teacher_data.DegreeLevelNumeric[i]))^rho +
                               (1 - value(delta)) * teacher_data.ExperienceYears[i]^rho)^(1/rho) for i in 1:nrow(teacher_data)
)
