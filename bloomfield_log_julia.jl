...
@NLobjective(model, Max, sum((delta * (b + c * log(1 + alpha * teacher_data.ExperienceYears[i] + beta * teacher_data.DegreeLevelNumeric[i]))^rho +
                               (1 - delta) * teacher_data.ExperienceYears[i]^rho)^(1/rho) for i in 1:nrow(teacher_data)))

# Use the full budget exactly
@NLconstraint(model, sum(b + c * log(1+alpha * teacher_data.ExperienceYears[i] + beta * teacher_data.DegreeLevelNumeric[i]) for i in 1:nrow(teacher_data)) == value(total_budget))

# Solve the optimization problem
optimize!(model)

optimized_total_salary = sum(value(b) + value(c) * log(1 + value(alpha) * teacher_data.ExperienceYears[i] + value(beta) * teacher_data.DegreeLevelNumeric[i]) for i in 1:nrow(teacher_data))
optimized_total_effectiveness = sum(
    (value(delta) * (value(b) + value(c) * log(1 + value(alpha) * teacher_data.ExperienceYears[i] + value(beta) * teacher_data.DegreeLevelNumeric[i]))^rho +
